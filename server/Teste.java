...
public class Teste {
	...
	public static void main(String[] args) throws IOException {
		...
		Date dataAbertura;
		int tipoContaCorrente, escolheAgConta, escolheConta, escolheContaTransacao, escolheCliente, codigo, senha,
				numeroConta;
		...
		while (true) {
			...
			DadosFuncionarios.cadastrar(new Gerente(CPFGerente, nomeCompletoGerente, dataNascimentoGerente,
					new Endereco(estadoGerente, cidadeGerente, bairroGerente, ruaGerente, numeroGerente),
					estadoCivilGerente, dataAdmissaoGerente, sexoGerente, cargoEmpresaGerente, ingressoGerenteGerente,
					cursoGerente));
		...
		while (true) {
			...
			DadosAgencias.cadastrar(new Agencia(numerodaAgencia, nomeAg,
					((Gerente) DadosFuncionarios.getFuncionarios().get(escolheGerente)),
					new Endereco(estadoAgencia, cidadeAgencia, bairroAgencia, ruaAgencia, numeroAgencia)));
		...
		while (true) {
			...
			DadosClientes.cadastrar(new Cliente(CPFCliente, nomeCliente, dataNascimentoCliente,
					new Endereco(estadoCliente, cidadeCliente, bairroCliente, ruaCliente, numeroCliente),
					estadocivilCliente, escolaridadeCliente, DadosAgencias.getAgencias().get(escolheAgencia)));
		...
		while (true) {
			...
			System.out.printf(
					"Informe 1 para conta corrente, 2 para conta poupanca e 3 para conta salario ou -1 para encerrar:\n");
			...
			if (escolheConta == 1) {
				...
				DadosContas.cadastrar(new Corrente(codigo, flagueAtiva, senha, numeroConta, saldoAtual, dataAbertura,
						DadosAgencias.getAgencias().get(escolheAgConta),
						DadosClientes.getClientes().get(escolheCliente), tipoContaCorrente));
			...
			if (escolheConta == 2) {
				...
				DadosContas.cadastrar(new Poupanca(codigo, flagueAtiva, senha, numeroConta, saldoAtual, dataAbertura,
						DadosAgencias.getAgencias().get(escolheAgConta),
						DadosClientes.getClientes().get(escolheCliente)));
			...
			if (escolheConta == 3) {
				...
				DadosContas.cadastrar(new Salario(codigo, flagueAtiva, senha, numeroConta, saldoAtual, dataAbertura,
						DadosAgencias.getAgencias().get(escolheAgConta),
						DadosClientes.getClientes().get(escolheCliente)));
		...
		while (true) {
			...
			if (escolheTransacao == 1) {
				...
				Transacao transacao = new Transacao(DadosAgencias.getAgencias().get(escolheAgencia), valorTransacao,
						tipoTransacao, DadosContas.getContas().get(escolheContaTransacao));
