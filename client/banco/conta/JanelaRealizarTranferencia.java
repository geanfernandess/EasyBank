...
public class JanelaRealizarTranferencia extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					JanelaRealizarTranferencia frame = new JanelaRealizarTranferencia();
	 ...
	 * Create the frame.
	 */
	public JanelaRealizarTranferencia() {
		...
		JLabel lbl_titulo = new JLabel("TRANFERENCIA");
		lbl_titulo.setFont(new Font("Tahoma", Font.BOLD, 11));
		lbl_titulo.setBounds(204, 11, 107, 14);
		contentPane.add(lbl_titulo);

		JLabel lbl_cpf = new JLabel("Digite o valor a ser transferido:");
		...
		JButton btn_sacar = new JButton("TRANSFERIR");
		btn_sacar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				double valorTransferencia = Double.parseDouble(c_valorTransacao.getText());
				...
				if (DadosAgencias.buscar(nroAgencia) != null && DadosContas.buscar(nroConta) != null &&
						DadosContas.buscar(nroConta).getSaldoAtual() > valorTransferencia) {
					Transacao transacao = new Transacao(DadosAgencias.buscar(nroAgencia), valorTransferencia, 'T',
							DadosContas.buscar(nroConta));

					// Professora, pelo que eu entendi da sua explicacao o objeto conta deveria
					// chamar o metodo efetiva transacao, feita da seguinte forma:
					// DadosContas.buscar(nroConta).setRefTransacao(transacao);
					// DadosContas.buscar(nroConta).getRefTransacao().get(0).efetivaTransacao();

					// Por�m quando eu seto uma transacao dentro do objeto conta eu n�o consigo
					// fazer a persistencia dos dados da conta,
					// sendo assim a unica maneira que consegui realizar a transacao e persistir os
					// dados da conta, foi da forma feita abaixo:

					transacao.efetivaTransacao();
					DadosContas.atualizarSaldo(nroConta);
					lbl_resultado.setText("Transferencia realizada com sucesso");
					lbl_saldoAtual.setText("O saldo atual e: " + DadosContas.buscar(nroConta).getSaldoAtual());
				}
		...
		JLabel lbl_statusSaque = new JLabel("Status transferencia:");
