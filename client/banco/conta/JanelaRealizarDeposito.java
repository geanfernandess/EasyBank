...
public class JanelaRealizarDeposito extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					JanelaRealizarDeposito frame = new JanelaRealizarDeposito();
	...
	public JanelaRealizarDeposito() {
		...
		JLabel lbl_titulo = new JLabel("DEPOSITO");
		lbl_titulo.setFont(new Font("Tahoma", Font.BOLD, 11));
		lbl_titulo.setBounds(204, 11, 107, 14);
		contentPane.add(lbl_titulo);

		JLabel lbl_cpf = new JLabel("Digite o valor a ser depositado:");
		...
		JButton btn_sacar = new JButton("DEPOSITO");
		btn_sacar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				double valorDeposito = Double.parseDouble(c_valorTransacao.getText());
				...
				if (DadosAgencias.buscar(nroAgencia) != null && DadosContas.buscar(nroConta) != null) {
					Transacao transacao = new Transacao(DadosAgencias.buscar(nroAgencia), valorDeposito, 'D',
					...
					lbl_resultado.setText("Deposito realizado com sucesso");
					lbl_saldoAtual.setText("O saldo atual e: " + DadosContas.buscar(nroConta).getSaldoAtual());
				}
		...
		JLabel lbl_statusSaque = new JLabel("Status deposito:");
